...
class AnnualAssessmentCertificatesAddProcess implements TwoStepProcessInterface, AutoWireableInterface
{
    ...
    {
        return $this->annualAssessmentCertificatesMapper->createAnnualAssessmentCertificate(
            $this->context->getTargetPersonId(),
            $this->context->getGroup(),
            $this->mapFormToDto($formData)
        );
    }
    ...
    {
        return sprintf('Group %s annual assessment certificate added successfully.', $this->context->getGroup());
    }
    ...
    {
        return sprintf('Add your group %s assessment certificate', $this->context->getGroup());
    }
    ...
    private function getAddPageUrl()
    {
    ...
    {
        return $this->certificatesPermissions->isGrantedCreate(
            $this->context->getTargetPersonId(),
    ...
    {
        $route = $this->annualAssessmentCertificatesRoutes->getAddReviewRoute();
        $params = $this->context->getController()->params()->fromRoute() + [
                self::ROUTE_PARAM_ID => $this->context->getTargetPersonId(),
                self::ROUTE_PARAM_GROUP => $this->context->getGroup(),
