...
class StartTestChangeService
{
    ...
    public function updateChangedValueStatus($value, $status)
    {
        ...
        $changes = (!empty($sessionStore[StartTestSessionService::ALLOWED_CHANGES]))
            ? $sessionStore[StartTestSessionService::ALLOWED_CHANGES] : [];
        ...
        $sessionStore[StartTestSessionService::ALLOWED_CHANGES] = $changes;
    ...
    {
        if (!in_array($value, $this->getAllowedChanges())) {
    ...
    {
        if (!in_array($change, $this->getAllowedChanges())) {
    ...
    public function loadAllowedChangesIntoSession()
    {
        ...
        foreach ($this->getAllowedChanges() as $allowedChange) {
            $allowedChanges[$allowedChange] = self::VALUE_NOT_CHANGED;
        }

        $sessionStore[StartTestSessionService::ALLOWED_CHANGES] = $allowedChanges;
        $this->startTestSessionService->save(StartTestSessionService::UNIQUE_KEY, $sessionStore);
    }
    ...
    public function isValueChanged($changedValue)
    {
        $sessionStore = $this->startTestSessionService->load(StartTestSessionService::UNIQUE_KEY);
        $changes = (!empty($sessionStore[StartTestSessionService::ALLOWED_CHANGES]))
            ? $sessionStore[StartTestSessionService::ALLOWED_CHANGES] : null;
     ...
     * @return mixed
     */
    public function underTestReturnUrl($obfuscatedVehicleId)
    {
     ...
     * @return array
     */
    public function getAllowedChanges()
    {
        return [
            self::CHANGE_CLASS,
            self::CHANGE_COLOUR,
