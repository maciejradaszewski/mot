...
class SecurityQuestionTwoStep extends AbstractRegistrationStep
{
    /**
     * const used for the session key, available via getId().
     */
    const STEP_ID = "SECURITY_QUESTION_TWO";

    const QUESTIONS_GROUP = "securityQuestionsGroupB";
    ...
    public function readFromArray(array $values)
    {
        if (is_array($values) && count($values)) {
            $this->setQuestion($values[SecurityQuestionSecondInputFilter::FIELD_QUESTION]);
            $this->setAnswer($values[SecurityQuestionSecondInputFilter::FIELD_ANSWER]);
        }
    }
    ...
    {
        return [
            SecurityQuestionSecondInputFilter::FIELD_QUESTION => $this->getQuestion(),
            SecurityQuestionSecondInputFilter::FIELD_ANSWER   => $this->getAnswer(),
        ];
    }
    ...
    {
        return 'account-register/security-question-two';
    }
