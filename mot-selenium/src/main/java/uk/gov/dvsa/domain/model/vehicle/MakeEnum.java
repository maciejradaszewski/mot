...
public enum MakeEnum {
    ...
    private MakeEnum(Integer makeId, String makeCode, String makeName) {
        id = makeId;
        code = makeCode;
        name = makeName;
    }
    ...
    public static MakeEnum findByName(String name) {
        for(MakeEnum make : values()){
            if( make.getName().equals(name)){
                return make;
            }
        }

        throw new LookupNamingException("MakeEnum " + name + " not found");
    }
}
