...
Feature: TQI Component Fail Rate
  ...
  Scenario: Get the tester performance statistics at site for the last month
    ...
    And there is no component fail rate statistics performed "1" months ago at site "Fast cars garage" for tester "Sikorsky" and group "A"
  ...
  Scenario: Get the tester performance statistics for the last month
    Given there are tests with reason for rejection performed at site "Slow cars garage" by "Kowalsky"
    When I am logged in as a Tester "Kowalsky"
    Then I should be able to see component fail rate statistics performed "1" months ago for tester "Kowalsky" and group "A"
    And I should be able to see component fail rate statistics performed "1" months ago for tester "Kowalsky" and group "B"
  ...
  Scenario: Get the tester performance statistics for the last month after changing AE
    ...
    And site "Fast cars garage" is linked to AE "Big Wheels" on "last day of previous month"
    When I am logged in as a Tester "Kowalsky"
    Then there is no component fail rate statistics performed "1" months ago for tester "Kowalsky" and group "A"
    Then there is no component fail rate statistics performed "1" months ago for tester "Kowalsky" and group "B"
