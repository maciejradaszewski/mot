...
{
    const HTTP_VOSA_MOT_API_8080 = UrlBuilder::MAIN_URL;
    ...
    {
        $this->assertSame(
            self::HTTP_VOSA_MOT_API_8080 . 'session/12',
            (new UrlBuilder())->session()->routeParam('id', 12)->toString()
        );
    }
    ...
    public function testShouldBuildUrlWithTwoParametersProvided()
    {
        $this->assertSame(
            ...
            (new UrlBuilder())->authorisedExaminerDesignatedManager()->routeParam('uuid', 12)->routeParam('aedmId', 13)
                ->toString()
        );
    }
