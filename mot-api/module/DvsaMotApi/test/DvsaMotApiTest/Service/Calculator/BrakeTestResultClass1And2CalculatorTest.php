...
class BrakeTestResultClass1And2CalculatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * Test lock percentage calculations.
     *
     * @dataProvider brakeTestResultItemsLockPercentage
     */
    public function testBrakeTestResultCalculatorForClasses1And2CheckLockPercentages($brakeTestResultItem)
    {
        $weights = $brakeTestResultItem['weights'];
        $efforts = $brakeTestResultItem['efforts'];
        $locks = $brakeTestResultItem['locks'];
        $message = $brakeTestResultItem['message'];
        $lockPercentage = $brakeTestResultItem['lockPercentage'];
        ...
        $brakeTestResult
            ...
            ->setControl1EffortFront($efforts[0])
            ->setControl1EffortRear($efforts[1])
            ->setControl2EffortFront($efforts[2])
            ->setControl2EffortRear($efforts[3])
            ->setControl1LockFront($locks[0])
            ->setControl1LockRear($locks[1])
            ->setControl2LockFront($locks[2])
            ->setControl2LockRear($locks[3]);
        $brakeTestResultCalculator = new BrakeTestResultClass1And2Calculator();
        $brakeTestResultCalculator->calculateBrakeTestResult($brakeTestResult, new \DateTime());

        $actualControl1PercentLocked = $brakeTestResultCalculator->calculateControl1PercentLocked($brakeTestResult);
        $actualControl2PercentLocked = $brakeTestResultCalculator->calculateControl2PercentLocked($brakeTestResult);

        $this->assertEquals($lockPercentage[0], $actualControl1PercentLocked, $message);
        $this->assertEquals($lockPercentage[1], $actualControl2PercentLocked, $message);
    }

    public static function brakeTestResultItemsLockPercentage()
    {
        $data = [
            [
                [
                    ...
                    'weights' => [500, 500, 0, 0],
                    'efforts' => [500, 500, 500, 500],
                    'locks' => [true, true, true, false],
                    'message' => 'Two locks on front, one lock on rear',
                    'lockPercentage' => [100, 50],
                ],
            ],
            [
                [
                    'type' => BrakeTestTypeCode::PLATE,
                    'weights' => [500, 500, 0, 0],
                    'efforts' => [500, 500, 500, 500],
                    'locks' => [true, false, false, false],
                    'message' => 'One lock on front, no locks on rear',
                    'lockPercentage' => [50, 0],
                ],
            ],
            [
                [
                    ...
                    'weights' => [500, 500, 0, 0],
                    'efforts' => [500, 500, 500, 500],
                    'locks' => [true, true, false, false],
                    'message' => 'Floor Type lock calculation is a special case for % lock calculations: one lock on front, no lock on rear',
                    'lockPercentage' => [100, 0],
                ],
            ],
            [
                [
                    ...
                    'weights' => [500, 500, 0, 0],
                    'efforts' => [500, 500, 500, 500],
                    'locks' => [false, false, true, true],
                    'message' => 'Floor Type lock calculation is a special case for % lock calculations: no lock on front, one lock on rear',
            ...
            [
                [
                    ...
                    'weights' => [500, 500, 0, 0],
                    'efforts' => [500, 500, 1, null],
                    'locks' => [true, true, true, true],
                    'message' => 'Two locks on rear, only one effort value rear: this state should be caught by api validation, but if not max calculator % should be 100',
                    'lockPercentage' => [100, 100],
                ],
            ],
        ];
