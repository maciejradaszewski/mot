...
class SiteTesterStatisticsTest extends \PHPUnit_Framework_TestCase
{
    /** @var SiteStatisticsService */
    private $service;

    /** @var SiteStatisticsRepository | \PHPUnit_Framework_MockObject_MockObject */
    private $repository;
    ...
    /** @var MethodSpy */
    private $repositorySpy;

    public function setUp()
    {
        /** @var SiteStatisticsRepository | \PHPUnit_Framework_MockObject_MockObject repository */
        $this->repository = XMock::of(SiteStatisticsRepository::class);

        $this->repositorySpy = new MethodSpy($this->repository, 'get');
        ...
        $this->service = new SiteStatisticsService($this->repository, $this->authorisationService, $this->getDateTimeHolder());
    }
    ...
    public function testAuthorisationIsRequiredToViewSiteStatistics()
    {
        ...
        $this->service->get($this->siteId, $this->getYear(), $this->getPrevMonth());
    ...
    public function testCalculateTotalTestCountForTester()
    {
        ...
        $this->addRepositoryRow($this->user1, $this->groupA, 2, 0, 0);

        // AND 3 group B tests
        $this->addRepositoryRow($this->user1, $this->groupB, 3, 0, 0);

        // AND another tester did 4 group A tests
        $this->addRepositoryRow($this->user2, $this->groupA, 4, 0, 0);

        // AND 10 group B tests
        $this->addRepositoryRow($this->user2, $this->groupB, 10, 0, 0);

        // WHEN I get statistics
        $stats = $this->getStatistics($this->siteId, $this->getYear(), $this->getPrevMonth());
    ...
    public function testStatisticsAreRetrievedForCorrectDateAndSite()
    {
        // GIVEN
        $year = $this->getYear();
        ...
        // WHEN I get statistics
        $this->getStatistics($this->siteId, $year, $month);

        // THEN they repository is asked for data
        $this->assertEquals(1, $this->repositorySpy->invocationCount());

        // for correct site
        $this->assertEquals($this->siteId, $this->repositorySpy->paramsForLastInvocation()[0]);

        // AND correct year
        $this->assertEquals($year, $this->repositorySpy->paramsForLastInvocation()[1]);

        // AND correct month
        $this->assertEquals($month, $this->repositorySpy->paramsForLastInvocation()[2]);
    }

    public function testCalculateFailurePercentage()
    {
        // GIVEN one tester did 2 group A tests and failed 0
        $this->addRepositoryRow($this->user1, $this->groupA, 2, 0, 0);

        // AND 5 group B tests and failed 1
        $this->addRepositoryRow($this->user1, $this->groupB, 5, 1, 0);

        // AND another tester did 6 group A tests and failed 6
        $this->addRepositoryRow($this->user2, $this->groupA, 6, 6, 0);

        // AND 0 group B
        $this->addRepositoryRow($this->user2, $this->groupB, 0, 0, 0);
     ...
     */
    public function testCalculateAverageVehicleAge(
    ...
    )
    {
        ...
        $this->addRepositoryRow($this->user1, $this->groupA, $tester1testCount, 0, 0, $tester1AverageVehicleAge, $tester1IsAverageAvailable);

        // AND another tester did tests in VTS
        $this->addRepositoryRow($this->user2, $this->groupA, $tester2testCount, 0, 0, $tester2AverageVehicleAge, $tester2IsAverageAvailable);
    ...
    public function testCalculateAverageTestTime()
    {
        // GIVEN one tester did 2 group A tests in total time 2:00
        $this->addRepositoryRow($this->user1, $this->groupA, 2, 0, 120);

        // AND 5 group B tests in 20 days
        $this->addRepositoryRow($this->user1, $this->groupB, 5, 0, 20 * 24 * 60 * 60);

        // AND another tester did 6 group A in one hour
        $this->addRepositoryRow($this->user2, $this->groupA, 6, 0, 3600);
    ...
    private function getStatistics($siteId, $year, $month)
    {
        $this->repositorySpy->mock()->willReturn($this->dbResult);
        return $this->service->get($siteId, $year, $month);
    }
