...
SET @permission_to_update_business_address_code = 'AE-UPDATE-BUSINESS-ADDRESS';
SET @permission_to_update_business_email_code = 'AE-UPDATE-BUSINESS-EMAIL';
SET @permission_to_update_business_phone_code = 'AE-UPDATE-BUSINESS-PHONE';
...
INSERT INTO `permission` (`name`, `code`, `created_by`, `created_on`, `last_updated_by`, `last_updated_on`) VALUES
  ('User can update address in AE business', @permission_to_update_business_address_code, @app_user_id, CURRENT_TIMESTAMP(6), @app_user_id, CURRENT_TIMESTAMP(6)),
  ('User can update email in AE business', @permission_to_update_business_email_code, @app_user_id, CURRENT_TIMESTAMP(6), @app_user_id, CURRENT_TIMESTAMP(6)),
  ('User can update telephone in AE business', @permission_to_update_business_phone_code, @app_user_id, CURRENT_TIMESTAMP(6), @app_user_id, CURRENT_TIMESTAMP(6)),
...
SET @permission_to_update_business_address = (SELECT `id` FROM `permission` WHERE `code` = @permission_to_update_business_address_code);
SET @permission_to_update_business_email = (SELECT `id` FROM `permission` WHERE `code` = @permission_to_update_business_email_code);
SET @permission_to_update_business_phone = (SELECT `id` FROM `permission` WHERE `code` = @permission_to_update_business_phone_code);
...
INSERT INTO `role_permission_map` (`role_id`, `permission_id`, `created_by` ,`created_on`, `last_updated_by`, `last_updated_on`)
  SELECT `role_id`, `permission_id`, @app_user_id, CURRENT_TIMESTAMP (6), @app_user_id, CURRENT_TIMESTAMP (6) FROM
    (
      SELECT @scheme_manager AS `role_id`, @permission_to_update_business_address AS `permission_id` UNION
      SELECT @scheme_user, @permission_to_update_business_address UNION
      SELECT @ao1, @permission_to_update_business_address UNION
      SELECT @ao2, @permission_to_update_business_address UNION
      SELECT @ve, @permission_to_update_business_address UNION

      SELECT @scheme_manager, @permission_to_update_business_email UNION
      SELECT @scheme_user, @permission_to_update_business_email UNION
      SELECT @ao1, @permission_to_update_business_email UNION
      SELECT @ao2, @permission_to_update_business_email UNION
      SELECT @ve, @permission_to_update_business_email UNION
      SELECT @aedm, @permission_to_update_business_email UNION
      SELECT @aed, @permission_to_update_business_email UNION

      SELECT @scheme_manager, @permission_to_update_business_phone UNION
      SELECT @scheme_user, @permission_to_update_business_phone UNION
      SELECT @ao1, @permission_to_update_business_phone UNION
      SELECT @ao2, @permission_to_update_business_phone UNION
      SELECT @ve, @permission_to_update_business_phone UNION
      SELECT @aedm, @permission_to_update_business_phone UNION
      SELECT @aed, @permission_to_update_business_phone UNION
