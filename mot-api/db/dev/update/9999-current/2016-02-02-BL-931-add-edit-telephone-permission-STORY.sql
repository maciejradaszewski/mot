/**
 * BL-931: New permission to edit telephone number
 */
...
SET @permission__edit_telephone_number = (SELECT `id` FROM `permission` WHERE `code` = 'EDIT-TELEPHONE-NUMBER');

/* Associate the new permission with selected roles */
INSERT INTO
  `role_permission_map` (`role_id`, `permission_id`, `created_by`, `last_updated_by`, `created_on`, `last_updated_on`)
VALUES
  (
    @role__ao1,
    @permission__edit_telephone_number,
    @app_user_id,
    @app_user_id,
    CURRENT_TIMESTAMP(6),
    CURRENT_TIMESTAMP(6)
