...
FOR EACH ROW
  BEGIN
    ...
    END IF;
    SET NEW.`version` = 1, NEW.`created_by` = IF(@app_user_id IS NOT NULL, @app_user_id, NEW.`created_by`), NEW.`last_updated_by` = IF(@app_user_id IS NOT NULL, @app_user_id, NEW.`last_updated_by`);
  END
$$
...
FOR EACH ROW
  BEGIN
    ...
    IF NEW.`status_id` <> ACTIVE_STATUS_ID AND NEW.`completed_date` IS NULL THEN
      SIGNAL SQLSTATE '10001' SET MESSAGE_TEXT = 'completed_date needs to be set for non ACTIVE mot tests';
    END IF;
    SET NEW.`version` = OLD.`version` + 1, NEW.`last_updated_by` = IF(@app_user_id IS NOT NULL, @app_user_id, NEW.`last_updated_by`);
  END
$$
