...
set @GB_regex = '^([A-Z]{5}|[A-Z]{4}9|[A-Z]{3}9{2}|[A-Z]{2}9{3}|[A-Z]9{4})[0-9]([05][1-9]|[16][0-2])(0[1-9]|[12][0-9]|(?<![05]2)30|(?<!([05][2469]|[16]1))31)[0-9][A-Z][A-Z9][0-9][A-Z]{2}$';

set @GB_country_code = (SELECT `id` FROM `licence_country_lookup` WHERE `code` = 'GB');
set @NI_country_code = (SELECT `id` FROM `licence_country_lookup` WHERE `code` = 'NI');
set @Non_UK_country_code = (SELECT `id` FROM `licence_country_lookup` WHERE `code` = 'NU');

UPDATE `licence`
SET `licence_country_id` =
CASE
  WHEN `licence_number` LIKE '[0-9]{8}'
  THEN @NI_country_code
  WHEN `licence_number` LIKE @GB_regex
  THEN @GB_country_code
  ELSE @Non_UK_country_code
END;
